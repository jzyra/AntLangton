...
Window::Window(const Config &config) {
	_antLangton = new AntLangton(config);
	_nbX = config.getXNb();
	_nbY = config.getYNb();
...
void Window::draw() {
	...
	bool **matrice = _antLangton->getMatrice();
	//Foreach cellulars.
	for (int i=0; i < _nbY; ++i) {
		for (int j=0; j < _nbX; ++j) {
			if (_antLangton->getXAnt() == i && _antLangton->getYAnt() == j) {
				//Loop for draw square.
				for(int x = j * sizeX; x < (j * sizeX)+sizeX; x++) {
					for(int y = i * sizeY; y < (i * sizeY)+sizeY; y++) {
						pixels[y*_width+x] = SDL_MapRGB(_surface->format, 255, 0, 0);
					}
				}    
			//If cellular is dead.
			} else if(matrice[i][j]) {

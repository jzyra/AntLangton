...
void Config::checkError() {
	...
	if (!(_xAnt < _XNb)) {
		cerr << "[-] Error, ant's position in x must be < at " << _XNb << endl;
		exit(1);
	}
	if ((_xAnt < 0)) {
		cerr << "[-] Error, ant's position in x must be > at -1 " << endl;
		exit(1);
	}
	if (!(_yAnt < _YNb)) {
		cerr << "[-] Error, ant's position in y must be < at " << _YNb << endl;
		exit(1);
	}
	if ((_yAnt < 0)) {
		cerr << "[-] Error, ant's position in y must be > at -1 " << endl;
		exit(1);
	}
	if (_height < 1) {
...
void Config::parseJson() {
	...
	json_object_object_foreach(_json, key, val) {
		...
		} else if (string(key) == "pos_x_ant") {
			if (type == json_type_int) {
				_xAnt = json_object_get_int(val);
			}
		} else if (string(key) == "pos_y_ant") {
			if (type == json_type_int) {
				_yAnt = json_object_get_int(val);
			}
		}
	}
}
...
int Config::getXAnt() const {
	return _xAnt;
}

int Config::getYAnt() const {
	return _yAnt;
}
